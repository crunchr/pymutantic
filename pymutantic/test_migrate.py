...
def test_migration_v2_to_v4():
    ...
    assert doc.state.some_field == "world"
    assert doc.state.some_new_field == 42.0
    assert doc.state.another_new_field is True
...
def test_migration_v3_to_v5():
    ...
    assert doc.state.some_new_field == 42.0
    assert doc.state.another_new_field is True
    assert doc.state.yet_another_field == 100
...
def test_concurrent_edit_during_migration():
    ...
    assert doc_v5.state.another_new_field is True
    assert doc_v5.state.yet_another_field == 100
    ...
    assert doc_v5_with_edit.state.schema_version == 5
    assert doc_v5_with_edit.state.some_field == "earth"
    assert doc_v5_with_edit.state.some_new_field == 42.0
    assert doc_v5_with_edit.state.another_new_field is True
    assert doc_v5_with_edit.state.yet_another_field == 100
